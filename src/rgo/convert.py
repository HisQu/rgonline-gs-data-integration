...
def inner_text(elem: ET.Element) -> str:
    """
    Return the concatenated textual content of an element, including nested text.
    """
...
def date_uri(lemma_id: str, idx: int) -> URIRef:
    return RG[f"date/{lemma_id}-{idx}"]


def fund_uri(lemma_id: str, idx: int) -> URIRef:
    return RG[f"fund/{lemma_id}-{idx}"]
...
def build_graph() -> Graph:
    ...
    for lemma in lemma_elements:
        ...
        g.add((lemma_node, EX.sourceId, Literal(lid)))
        ...
        if reg is not None:
            ...
            for idx, sub in enumerate(sublemmata, start=1):
                ...
                g.add((lemma_node, EX.hasSubEntry, sub_node))
                g.add((sub_node, EX.partOfLemma, lemma_node))
                ...
                if date_el is not None:
                    d_node = date_uri(lid, idx)
                    g.add((d_node, RDF.type, EX.DateInfo))
                    g.add((sub_node, EX.hasDate, d_node))
                ...
                fund_el = sub.find("fund")
                if fund_el is not None:
                    f_node = fund_uri(lid, idx)
        ...
        if personenindex is not None:
            for person in personenindex.findall("person"):
                ...
                if name_el is not None:
                    add_literal_if_present(g, person_node, EX.preferredName, inner_text(name_el))

                beiname_el = person.find("beiname")
                if beiname_el is not None:
                    add_literal_if_present(g, person_node, EX.variantName, inner_text(beiname_el))
